  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5 pb-6">
      ...
      <div className="bg-royal-gradient rounded-3xl p-6 sm:p-8 relative overflow-hidden shadow-royal-lg">
        <div className="absolute top-0 right-0 w-48 h-48 rounded-full bg-gold/10 blur-[40px] pointer-events-none" />
      ...
      {profile?.loyalty_points != null && (
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-white border border-border rounded-2xl p-5 text-center hover:shadow-royal hover:-translate-y-0.5 transition-all duration-300">
            ...
            <p className="font-syne font-bold text-2xl text-gold">{profile.loyalty_points.toLocaleString('en-IN')}</p>
            <p className="text-xs text-muted mt-1 font-medium">Loyalty Points</p>
          </div>
          <Link href="/loyalty" className="bg-white border border-border rounded-2xl p-5 text-center hover:border-gold/40 hover:shadow-royal hover:-translate-y-0.5 transition-all duration-300 block">
            ...
            <p className="text-xs text-gold mt-1 font-bold">View Rewards →</p>
          </Link>
        </div>
      )}
      ...
      <div className="bg-white border border-border rounded-2xl overflow-hidden shadow-sm">
        {MENU_ITEMS.map((item, idx) => (
          ...
          >
            <div className="w-10 h-10 rounded-xl bg-lavender flex items-center justify-center shrink-0 text-navy group-hover:bg-gold/10 group-hover:text-gold transition-colors">
              {item.icon}
            </div>
            <div className="flex-1 min-w-0">
            ...
            <svg className="w-4 h-4 text-muted group-hover:text-gold group-hover:translate-x-0.5 transition-all" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
