  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-syne text-2xl font-bold text-saloo-dark">Analytics</h1>
          <p className="text-saloo-dark/50 text-sm mt-0.5">Performance overview</p>
        </div>
          ...
          {PERIODS.map(p => (
            <button key={p.key} onClick={() => setPeriod(p.key)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                period === p.key ? 'bg-saloo-pink text-saloo-cream' : 'text-saloo-dark/60 hover:text-saloo-dark'
              }`}>
              {p.label}
            </button>
          ))}
      ...
      {isLoading ? (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {Array.from({ length: 8 }).map((_, i) => <div key={i} className="h-24 bg-white/60 backdrop-blur-md shadow-sm border border-white/80 rounded-2xl animate-pulse" />)}
        </div>
      ) : data ? (
        <div className="space-y-6">
          ...
          <div className="bg-white/60 backdrop-blur-md shadow-sm border border-white/80 rounded-2xl p-5">
            <p className="text-saloo-dark/50 text-xs uppercase tracking-widest mb-5">Booking Rates</p>
            <div className="space-y-5">
              <RateBar label="Completion"  value={data.completion_rate}  color="bg-green-500" />
              <RateBar label="Cancellation" value={data.cancellation_rate} color="bg-saloo-pink" />
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            ...
            {data.top_services?.length > 0 && (
              <div className="bg-white/60 backdrop-blur-md shadow-sm border border-white/80 rounded-2xl p-5">
                <p className="text-saloo-dark/50 text-xs uppercase tracking-widest mb-5">Top Services</p>
                  ...
                  {data.top_services.map((s: any, i: number) => (
                    <div key={i} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-saloo-dark/70 text-sm">{s.name}</span>
                      </div>
                      <span className="text-saloo-pink text-sm font-semibold">{s.count}× · {formatINR(s.revenue)}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {data.top_barbers?.length > 0 && (
              <div className="bg-white/60 backdrop-blur-md shadow-sm border border-white/80 rounded-2xl p-5">
                <p className="text-saloo-dark/50 text-xs uppercase tracking-widest mb-5">Top Barbers</p>
                <div className="space-y-3">
          ...
          {data.revenue_by_day?.length > 0 && (
            <div className="bg-white/60 backdrop-blur-md shadow-sm border border-white/80 rounded-2xl p-5">
              <p className="text-saloo-dark/50 text-xs uppercase tracking-widest mb-5">Daily Revenue</p>
              <div className="space-y-0 max-h-80 overflow-y-auto scrollbar-none">
                {data.revenue_by_day.slice(-14).reverse().map((d: any) => (
                  <div key={d.date} className="flex items-center justify-between py-2.5 border-b border-white/[0.05] last:border-0">
                    <span className="text-saloo-dark/60 text-sm">{new Date(d.date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}</span>
                    <div className="flex items-center gap-5">
                      <span className="text-saloo-dark/50 text-sm">{d.bookings} bookings</span>
                      <span className="text-saloo-pink font-semibold text-sm tabular-nums">{formatINR(d.revenue)}</span>
                    </div>
                  </div>
                ))}
              </div>
...
function Metric({ label, value, sub, accent }: { label: string; value: string; sub: string; accent?: boolean }) {
  return (
    <div className="bg-white/60 backdrop-blur-md shadow-sm border border-white/80 rounded-2xl p-5">
      <p className="text-saloo-dark/50 text-xs uppercase tracking-wider mb-3">{label}</p>
      <p className={`font-syne text-2xl font-bold ${accent ? 'text-saloo-pink' : 'text-saloo-dark'}`}>{value}</p>
      <p className="text-saloo-dark/50 text-xs mt-1.5">{sub}</p>
    </div>
  )
}
  ...
  return (
    <div>
      <div className="flex justify-between mb-2">
        <span className="text-saloo-dark/70 text-sm">{label}</span>
        <span className="text-saloo-dark font-semibold text-sm tabular-nums">{value}%</span>
      </div>
      <div className="h-1.5 bg-white/70 backdrop-blur-md shadow-sm rounded-full">
